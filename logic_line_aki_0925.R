...
#basic graph
ft <- gaze(AKI~.,data=df) %>%myft()
print(ft)
table2docx(ft,target=paste(output_dir,"Basic_Graph"))

#train and test data
ind <- createDataPartition(df$AKI, p=1, list = FALSE)
...
#选取部分变量进行拟合
#fit <- lrm(AKI~Gender + Age + BMI + cTnT + BNP + CKMB + Myo + Alb + TP + Cr + BUN + Hb + RBC + HCT + PLT + APTT + PT + D.D + HBP + DM + CVA + AMI + CKD + PE + Kidney + Eth + Bh + Th + RBCu + Plaml + Cry + PLTu + OP,
...
train_data$AKI <- as.numeric(train_data$AKI)-1
train_dc <- decision_curve(AKI~pred,
...
test_data$AKI <- as.numeric(test_data$AKI)-1
test_dc <- decision_curve(AKI~pred,
